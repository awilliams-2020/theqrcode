...
export default function TrialBanner({ trialEndsAt, status, currentPlan = 'pro', onUpgrade, onViewPlans }: TrialBannerProps) {
  ...
  const getPlanFeatures = (plan: string) => {
    switch (plan) {
      case 'starter':
        return t('starterFeatures')
      case 'pro':
        return t('proFeatures')
      case 'business':
        return t('businessFeatures')
      default:
      ...
      <div className="flex items-start">
        ...
        <div className="flex-1">
          <h3 className="text-sm font-semibold text-blue-800 mb-1">
            {t('freeTrialActive')} - {planDisplayNames[currentPlan as keyof typeof planDisplayNames]} {t('featuresUnlocked')}
          </h3>
          <p className="text-sm text-blue-700 mb-2">
            {message}. {t('trialActiveMessage', { plan: planDisplayNames[currentPlan as keyof typeof planDisplayNames], features: getPlanFeatures(currentPlan) })}
          </p>
          <div className="flex gap-2">
            ...
            >
              {t('upgradeToKeepFeatures', { plan: planDisplayNames[currentPlan as keyof typeof planDisplayNames] })}
            </button>
            <button
